...
export default {
    "create": function(logo, sys) {
        ...
        const KEYBOARD_EVENT_CODE_CODE = CONSTANTS.KEYBOARD_EVENT_CODE_CODE;

        const KEYBOARD_EVENT_KEY_CODE = CONSTANTS.KEYBOARD_EVENT_KEY_CODE;
        ...
        const methods = {
            ...
            "getfocus": primitiveGetfocus,

            "setfocus": primitiveSetfocus,
        ...
        function primitivePenpaint() {
            _penDown = true;
            _penMode = "paint";
            logo.canvas.sendCmd("penpaint");
        }

        function primitivePenerase() {
            _penDown = true;
            _penMode = "erase";
            logo.canvas.sendCmd("penerase");
        }

        function primitivePenreverse() {
            _penDown = true;
            _penMode = "reverse";
            logo.canvas.sendCmd("penreverse");
        }
        ...
        function primitiveShowturtle() {
            _showTurtle = true;
            logo.canvas.sendCmd("showturtle");
        }

        function primitiveHideturtle() {
            _showTurtle = false;
            logo.canvas.sendCmd("hideturtle");
        }
        ...
        function primitiveForward(distance) {
            logo.env.validateInputNumber(distance);
        ...
        function primitiveClearscreen() {
            ...
            logo.canvas.sendCmd("moveto", [_turtleX, _turtleY, _turtleHeading]);
            logo.canvas.sendCmd("clean");
        }
        ...
        function primitiveSetxy(newX, newY) {
            logo.env.validateInputNumber(newX);
            logo.env.validateInputNumber(newY);
            _turtleX = newX;
            _turtleY = newY;
            logo.canvas.sendCmd(_penDown ? "drawto" : "moveto", [_turtleX, _turtleY, _turtleHeading]);
        }
        ...
        function primitiveSetbackground(color) {
            logo.env.validateInputRGB(color);
            _bgColor = logo.type.getRGB(color);
            logo.canvas.sendCmd("bgcolor", _bgColor);
        }
        ...
        function primitiveSetfloodcolor(color) {
            ...
            logo.canvas.sendCmd("fillcolor", logo.type.getRGB(_floodColor));
        }
        ...
        async function primitiveGetfocus() {
            return await logo.io.call("getfocus");
        }

        function primitiveSetfocus(name) {
            logo.env.validateInputWordOrList(name);
            logo.io.call("setfocus", logo.type.toString(name));
        }
        ...
        function primitiveSetpencolor(color) {
            ...
            logo.canvas.sendCmd("pencolor", logo.type.getRGB(_penColor));
        }
        ...
        function primitiveSetpensize(size) {
            logo.env.validateInputPensize(size);
            ...
            logo.canvas.sendCmd("pensize", [actualSize]);
        }
        ...
        function primitiveCircle(radius, fill = false) {
            logo.env.validateInputNonNegNumber(radius);
            logo.env.validateInputBoolean(fill);
            if (_penDown) {
                logo.canvas.sendCmd("arc", [_turtleX, _turtleY, radius, 0, 360, false, logo.type.logoBoolean(fill)]);
            }
        }
        ...
        function primitiveEllipse(radiusX, radiusY, fill = false) {
            logo.env.validateInputNonNegNumber(radiusX);
            logo.env.validateInputNonNegNumber(radiusY);
            logo.env.validateInputBoolean(fill);
            if (_penDown) {
                logo.canvas.sendCmd("ellipse", [
                    _turtleX,
                    _turtleY,
                    radiusX,
                    radiusY,
        ...
        function primitiveEllipsearc(deg, radiusX, radiusY, startDeg) {
            ...
            logo.env.validateInputNonNegNumber(radiusX);
            logo.env.validateInputNonNegNumber(radiusY);
            logo.env.validateInputNumber(startDeg);
        ...
        function primitiveLabel(text) {
            ...
            logo.canvas.sendCmdAsString("drawtext", [logo.type.toString(text), canvasFont]);
        }
        ...
        function primitiveFill(fillmode = false) {
            logo.env.validateInputBoolean(fillmode);
            logo.canvas.sendCmd("fill", [logo.type.logoBoolean(fillmode)]);
        }
        ...
        function primitiveKeyboardon(onKeyDown, onKeyUp = undefined) {
            logo.env.validateInputWordOrList(onKeyDown);
            if (onKeyUp !== undefined) {
                logo.env.validateInputWordOrList(onKeyUp);
            }
        ...
        function primitiveMouseon(onLeftButtonDown, onLeftButtonUp, onRightButtonDown, onRightButtonUp, onMove) {
            logo.env.validateInputWordOrList(onLeftButtonDown);
            logo.env.validateInputWordOrList(onLeftButtonUp);
            logo.env.validateInputWordOrList(onRightButtonDown);
            logo.env.validateInputWordOrList(onRightButtonUp);
            logo.env.validateInputWordOrList(onMove);
            ...
            await logo.env.resetCallStackAfterExecute(async () => {
                await logo.env.resetScopeStackAfterExecute(async () => {
                    await logo.env.catchLogoException(async () => {
                        ...
                        logo.env.validateInputList(template);
