...
export default {
    "create": function(logo) {
        ...
        function primitiveModule(moduleName) {
            logo.env.throwIf(!logo.config.get("module"), logo.type.LogoException.NOT_ENABLED);
            logo.env.throwIf(!logo.env.inDefaultModule(), logo.type.LogoException.NOT_ALLOWD_IN_MODULE);
            logo.env.validateInputWord(moduleName);
            logo.env.setModule(moduleName);
        }

        function primitiveEndmodule() {
            logo.env.throwIf(!logo.config.get("module"), logo.type.LogoException.NOT_ENABLED);
            logo.env.throwIf(logo.env.inDefaultModule(), logo.type.LogoException.ONLY_IN_MODULE);
            logo.env.setModule();
        }
        ...
        function primitiveExport(procs) {
            logo.env.throwIf(!logo.config.get("module"), logo.type.LogoException.NOT_ENABLED);
            logo.env.validateInputList(procs);
        ...
        function primitiveIsa(superList) {
            logo.env.throwIf(logo.env.inDefaultModule(), logo.type.LogoException.ONLY_IN_MODULE);
            logo.env.validateInputList(superList);
