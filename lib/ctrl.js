...
export default {
    "create": function(logo, sys) {
        ...
        const methods = {
            ...
            "transfer": {jsFunc: primitiveTransfer, attributes: PROC_ATTRIBUTE.STASH_LOCAL_VAR | PROC_ATTRIBUTE.RETURNS_IN_LAMBDA},
            ...
            "?in": [primitiveQuestionMarkIn],

            "?out": [primitiveQuestionMarkOut],
        ...
        async function applyHelper(template, inputList, index = 1, unboxedRestList = [], unboxedMap = {}) {
            logo.env.validateInputList(inputList);

            let unboxedInputList = logo.type.unbox(inputList);
            let srcmap = logo.env.getProcSrcmap();
            let slot = logo.env.makeSlotObj(unboxedInputList, index, unboxedRestList, unboxedMap);
        ...
        async function primitiveForeach(...inputs) {
            ...
            inputs.forEach(logo.env.validateInputWordOrList);
        ...
        async function primitiveReduce(template, data) {
            logo.env.validateInputNonEmptyWordOrList(data);
        ...
        async function primitiveTransfer(endTest, template, inBasket) {
            logo.env.validateInputWordOrList(inBasket);
            let unboxedInBasket = splitWordOrUnboxLogoList(inBasket);

            let skipEndTest = logo.type.isEmptyList(endTest);
            let outBasket = logo.type.EMPTY_LIST;
            let isLogoWord = logo.type.isLogoWord(inBasket);
            let index = 1;
            while (unboxedInBasket.length > 0) {
                let inputList = logo.type.makeLogoList([outBasket, getRemainingInBasket()]);
                let map = {"in": unboxedInBasket.shift(), "out": outBasket};
                if (!skipEndTest && logo.type.isLogoBooleanTrue(await applyHelper(endTest, inputList, index, [], map))) {
                    break;
                }

                outBasket = await applyHelper(template, inputList, index, [], map);
                index++;
            }

            return outBasket;

            function getRemainingInBasket() {
                return isLogoWord ? inBasket.substr(index - 1) : logo.type.makeLogoList(unboxedInBasket);
            }
        }
        ...
        async function primitiveCrossmap(template, ...args) {
            if (args.length === 1) {
                logo.env.validateInputWordOrList(args[0]);
        ...
        async function primitiveFilter(template, data) {
            logo.env.validateInputWordOrList(data);
        ...
        async function primitiveRepeat(count, template) {
            logo.env.validateInputPosNumber(count);
            logo.env.validateInputList(template);
        ...
        async function primitiveMacroexpand(template) {
            template = logo.type.wordToList(template);
            logo.env.validateInputNonEmptyList(template);
            logo.env.validateInputMacro(logo.type.listFirst(template));
            return await logo.env.callTemplate(template, true, true);
        }

        async function primitiveIf(predicate, template) {
            logo.env.validateInputBoolean(predicate);
        ...
        async function primitiveWhile(predicate, template) {
            logo.env.validateInputList(predicate);
        ...
        async function primitiveIfelse(predicate, templateTrue, templateFalse) {
            ...
            logo.env.validateInputList(templateTrue);
            logo.env.validateInputList(templateFalse);
        ...
        async function primitiveCatch(label, template) {
            logo.env.validateInputWord(label);
            template = logo.type.wordToList(template);
            logo.env.validateInputList(template);
        ...
        function primitiveBye() {
            logo.env.setEnvState("exit");
        ...
        function primitiveQuestionMarkIn() {
            return logo.env.getSlotMapValue("in");
        }

        function primitiveQuestionMarkOut() {
            return logo.env.getSlotMapValue("out");
        }
